...
Future<Stream<Food>> getTrendingFoods(Address address, String city_id) async {
...
//   _queryParams['city'] = '14';
//   _queryParams = filter.toQuery(
//     oldQuery: _queryParams,
//   );
//   uri = uri.replace(queryParameters: _queryParams);
//   print(CustomTrace(StackTrace.current, message: uri.toString()).toString());
//   try {
...
Future<Stream<Restaurant>> getStoreByCategory(
    categoryId, String city_id) async {
  print(
  ...
  _queryParams['search'] = 'category_id:$categoryId';
  _queryParams['searchFields'] = 'category_id:=';
  // _queryParams[''] = '$city_id';
  _queryParams = filter.toQuery(oldQuery: _queryParams, city_id: city_id);
  uri = uri.replace(queryParameters: _queryParams);
  print(
      "Url for store by category ::::::::::::::::>>>>>>>>>> " + uri.toString());
  try {
    final client = new http.Client();
    final streamedRest = await client.send(http.Request('get', uri));
...
Future<Stream<Food>> getFoodsOfRestaurant(
    String restaurantId, String category_id) async {
  final String url =
      '${GlobalConfiguration().getString('api_base_url')}foods?category_id=$category_id&with=restaurant&search=restaurant.id:$restaurantId&searchFields=restaurant.id:=';
  try {
    final client = new http.Client();
    final streamedRest = await client.send(http.Request('get', Uri.parse(url)));
    print("Menu URl ::::::::::::::::::::::::::::::: " + url.toString());
    return streamedRest.stream
        .transform(utf8.decoder)
        .transform(json.decoder)
        .map((data) => Helper.getData(data))
